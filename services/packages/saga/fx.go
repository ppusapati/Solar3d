...
var SagaEngineModule = fx.Module(
	...
	//
	// The default provider returns the in-memory MockKafkaProducer so the
	// saga module is self-contained for tests and for environments that
	// don't run Kafka. Production wiring should override this with a real
	// producer by supplying its own `fx.Provide(func() events.KafkaProducer
	// { return realProducer })` AFTER importing this module — fx's
	// "last-write-wins" provider semantics let downstream apps swap it
	// without forking the saga module.
	fx.Provide(
