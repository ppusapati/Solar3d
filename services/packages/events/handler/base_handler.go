...
// Handle is the template-method hook overridden by concrete handlers. The
// base implementation deliberately returns an error so that registering a
// raw `BaseEventHandler` (without subclassing) fails fast at the first
// dispatched event rather than silently dropping work. Concrete handlers
// override this method to do the actual processing.
func (h *BaseEventHandler) Handle(ctx context.Context, event *domain.DomainEvent) error {
	return fmt.Errorf("event handler %q must override Handle(); base implementation is a guard against forgotten subclassing", h.handlerName)
}
