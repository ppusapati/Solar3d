...
func main() {
	logger := zerolog.New(os.Stdout).With().Timestamp().Str("service", "structural-service").Logger()
	...
	go func() {
		logger.Info().Str("addr", srv.Addr).Msg("structural-service listening")
	...
	logger.Info().Msg("structural-service stopped")
}
